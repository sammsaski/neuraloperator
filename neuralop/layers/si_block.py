...
class FastPhi1D(nn.Module):
    def __init__(self, embed_dim):
        super().__init__()
        self.weight = nn.Parameter(torch.randn(embed_dim))
        self.i_proj = nn.Linear(1, embed_dim, bias=False)
        self.j_proj = nn.Linear(1, embed_dim, bias=False)
...
class SIBlocks1D(nn.Module):
	...
	def __init__(
		...
		self.phi_net = FastPhi1D(embed_dim=in_channels)
	...
	def forward(self, x):
		...
		coords = torch.linspace(0, 1, N, device=x.device).view(1, N, 1).repeat(B, 1, 1)  # (B, N, 1)
		...
		psi_vals = SIBlocks.bspline_basis_1d(relative_coords[:, 0], self.knot_points, self.S_m)
