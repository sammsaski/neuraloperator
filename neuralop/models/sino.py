...
from ..layers.si_block import SIBlocks, SIBlocks1D
from ..layers.channel_mlp import ChannelMLP
from .base_model import BaseModel


class SINO1D(BaseModel, name='SINO1D'):
    ...
    def __init__(
      ...
      non_linearity: nn.Module=F.gelu
        ...
        # ])
        self.sino_blocks = nn.ModuleList([
            SIBlocks1D(in_channels=self.lifting_channels, out_channels=self.projection_channels, num_knots=self.num_knots)
            for _ in range(self.n_layers)
        ])
